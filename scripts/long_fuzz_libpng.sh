#!/bin/bash
# Long-duration fuzzing (10 hours) for libpng using best drivers.
# Goal: find new defects (memory bugs, crashes, undefined behavior).
#
# Usage: ./scripts/long_fuzz_libpng.sh
# Requires: Docker running, magic4fdg:latest image built.
#
# Runs in detached mode — use `docker logs -f magic4fdg-long-fuzz-libpng` to monitor.
# Output: generated/long_fuzz_libpng/

set -euo pipefail

SCRIPT_DIR="$(cd "$(dirname "$0")" && pwd)"
PROJECT_DIR="$(cd "$SCRIPT_DIR/.." && pwd)"
DRIVERS_DIR="$PROJECT_DIR/generated/iterations/libpng/latest/best_drivers"
OUTPUT_DIR="$PROJECT_DIR/generated/long_fuzz_libpng"
FUZZ_SECONDS=$((10 * 3600))  # 10 hours

echo "=== MAGIC4FDG Long Fuzz: libpng ==="
...
mkdir -p "$PROJECT_DIR/generated/long_fuzz_drivers_libpng"
cp "$DRIVERS_DIR"/*.cpp "$PROJECT_DIR/generated/long_fuzz_drivers_libpng/"

# Copy seed corpus
mkdir -p "$PROJECT_DIR/generated/long_fuzz_drivers_libpng/seed_corpus"
cp "$PROJECT_DIR/benchmarks/libs/libpng/seed_corpus"/* \
   "$PROJECT_DIR/generated/long_fuzz_drivers_libpng/seed_corpus/" 2>/dev/null || true
cp "$PROJECT_DIR/generated/accumulated_corpus/libpng"/* \
   "$PROJECT_DIR/generated/long_fuzz_drivers_libpng/seed_corpus/" 2>/dev/null || true

# Check dictionary
DICT_FILE="$PROJECT_DIR/benchmarks/libs/libpng/png.dict"
DICT_FLAG=""
if [ -f "$DICT_FILE" ]; then
    DICT_FLAG="-dict=/workspace/benchmarks/libs/libpng/png.dict"
fi

# Generate the in-container script
cat > "$PROJECT_DIR/generated/long_fuzz_script_libpng.sh" << INNEREOF
#!/bin/bash
set -e
...
DRIVERS_DIR="/workspace/generated/long_fuzz_drivers_libpng"
INCLUDE="-I/opt/bench/libpng/include/libpng16 -I/opt/bench/zlib/include"
LIBS="/opt/bench/libpng/lib/libpng16.a /opt/bench/zlib/lib/libz.a -lm"
...
for driver in "\$DRIVERS_DIR"/*.cpp; do
    ...
    if \$CXX -std=c++17 -g -O1 \\
        ...
        echo "  FAILED: \$name"
        cat "/tmp/logs/\${name}_compile.log" | tail -5
    fi
done
...
# Each driver gets its own corpus dir
echo "[*] Preparing per-driver corpus..."
for binary in /tmp/binaries/*; do
    name=\$(basename "\$binary")
    mkdir -p "/tmp/corpus_\$name"
    cp /workspace/generated/long_fuzz_drivers_libpng/seed_corpus/* "/tmp/corpus_\$name/" 2>/dev/null || true
done
...
for binary in /tmp/binaries/*; do
    ...
    ASAN_OPTIONS="halt_on_error=0:exitcode=0:detect_leaks=0:print_stacktrace=1:log_path=/tmp/logs/\${name}_asan" \\
...
# Copy results back
mkdir -p /workspace/generated/long_fuzz_libpng/crashes
mkdir -p /workspace/generated/long_fuzz_libpng/corpus
mkdir -p /workspace/generated/long_fuzz_libpng/logs

cp -r /tmp/crashes/* /workspace/generated/long_fuzz_libpng/crashes/ 2>/dev/null || true
for d in /tmp/corpus_*; do
    cp "\$d"/* /workspace/generated/long_fuzz_libpng/corpus/ 2>/dev/null || true
done
cp /tmp/logs/* /workspace/generated/long_fuzz_libpng/logs/ 2>/dev/null || true
...
chmod +x "$PROJECT_DIR/generated/long_fuzz_script_libpng.sh"

# Mount build cache
BUILD_CACHE="$PROJECT_DIR/generated/build_cache/libpng"
...
echo "Container: magic4fdg-long-fuzz-libpng"
echo ""

# Run detached: 8GB RAM, 4 CPUs
docker run -d \
    --name magic4fdg-long-fuzz-libpng \
    --memory 8g \
    --cpus 4 \
    ...
    -w /workspace \
    magic4fdg:latest \
    bash generated/long_fuzz_script_libpng.sh

echo ""
echo "=== Container started ==="
echo "Monitor:  docker logs -f magic4fdg-long-fuzz-libpng"
echo "Stop:     docker stop magic4fdg-long-fuzz-libpng"
